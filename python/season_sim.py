...
if(quali_exists):
    csv_name=f'{CSV_DIR}/post_q_race.csv'
#overwise save to base csv for race n
else:
    csv_name=f'{CSV_DIR}/pre_q_race.csv'
#Only if did a full run of 10000+
if(n>9999):
    single_race_pct_sorted.to_csv(csv_name)

#save overall standings results to csv
#if quali happened save to quali special csv for race n
if(quali_exists):
    csv_name=f'{CSV_DIR}/post_q_season.csv'
#overwise save to base csv for race n
else:
    csv_name=f'{CSV_DIR}/pre_q_season.csv'
#Only if did a full run of 10000+
if(n>9999):
    results_df_pct_sorted.to_csv(csv_name)
