...
function buildPredictionRows(resultsData) {
    ...
    const races = Object.entries(predictionData.races)
        .filter(([eventId]) =>
            predictionData.race_data.hasOwnProperty(eventId) &&
            resultsData.hasOwnProperty(eventId)
        )
        .sort((a, b) => a[1][0] - b[1][0]);

    races.forEach(([eventId, [, raceName]]) => {

        Object.entries(resultsData[eventId]).forEach(([driver, actual]) => {

            
            const prediction =
            predictionData.race_data[eventId][driver];
            ...
            rows.push({
                ...
                driver,
                driverName: prediction.driver_name,

                team: prediction.team_name,

                actualFinish: actual,
                expectedFinish: prediction.expected_finish,

                dnfChance: prediction.dnf_prob,
                dnfChanceDNFONLY: 
                    actual === "DNF"
                        ? prediction.dnf_probability*100
                        : null,
                finishOdds: 
                    actual === "DNF"
                        ? prediction.dnf_probability*100
                ...
                error:
                    actual === "DNF"
                        ? null
                        : Math.abs(actual - prediction.expected_finish)
...
function updateAME(checkBox,metric) {
    ...
    const previousSearch =
        document.getElementById("driverSearchBar")?.value || "";
    ...
    if (checkBox==="Driver") {
        ...
        searchbar.innerHTML = `
            <input
                ...
                class="driverBarSearch"
            >
        `;

        const searchInput =
            document.getElementById("driverSearchBar");
...
async function initialize() {
    ...
    allPredictionRows
        .filter(row => row.dnfChanceDNFONLY !== null)
        .sort((b,a) => a.dnfChanceDNFONLY - b.dnfChanceDNFONLY)
        .forEach((row,index) => {
            row.DNFRank = index + 1;
        });
    allPredictionRows
        .filter(row => row.finishOdds!== null)
        .sort((b,a) => a.finishOdds - b.finishOdds)
        .forEach((row,index) => {
            row.finishOddsRank = index + 1;
        });
