...
function updateDriverHeatmap(selectedRace) {
    ...
    Object.values(raceData).forEach(driverData => {
        for (let pos = 1; pos <= 22; pos++) {
            const prob = Number(driverData[String(pos)]["count"]) || 0;
    ...
    Object.keys(raceData).forEach(driver => {
        ...
        for (let pos = 1; pos <= 22; pos++) {
            ...
            const prob =
                raceData[driver][String(pos)]["count"];
        ...
        if(previousRaceData[driver][1]===0) {
            if(raceData[driver][1]===0) {
                driverList.push({
                    ...
                    lastwinProb: 0,
                    avgPoints: raceData[driver].avgPoints
                });
            } else {
                driverList.push({
                    driver: raceData[driver].driver_name,
                    expFinish: raceData[driver].expected_finish,
                    winProb: raceData[driver].win_probability,
                    lastwinProb: raceData[driver].win_probability-previousRaceData[driver].win_probability,
                    avgPoints: raceData[driver].avgPoints
                });
            }
        } else {
...
function buildDriverTable(driverList) {
    ...
    driverList.forEach(driver => {
        ...
        row.insertCell().textContent = driver.avgPoints.toFixed(2);
        ...
        if(previousRaceData[teamName][1]===0) {
            if(raceData[teamName][1]===0) {
                teamList.push({
                    ...
                    lastwinProb: 0,
                    avgPoints:raceData[teamName].avgPoints
                });
            } else {
                teamList.push({
                    team: teamName,
                    expFinish: raceData[teamName].expected_finish,
                    winProb: raceData[teamName].win_probability,
                    lastwinProb: raceData[teamName].win_probability-previousRaceData[teamName].win_probability,
                    avgPoints:raceData[teamName].avgPoints
                });
            }
        } else {
